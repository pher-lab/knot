...
**現在バージョン**: v0.6.0-alpha（リリース済み 2026-03-10）
...
- [x] 全テストパス確認（Rust 94 + Frontend 31 = 125）
- [x] TypeScript型チェック: エラーなし
- [x] プロダクションビルド: MSI + NSIS生成成功
- [x] コミット3件: パスワード変更 → タグ&セキュリティ&UX → バージョンバンプ
- [x] GitHub Release `v0.3.0-alpha` 作成（Pre-release、インストーラー2種添付）
- [x] Pure Rust GUI移行についての議論 → 現時点ではコスト対効果が合わないため保留
...
**メイン機能:**
- 画像付きエクスポート/インポート（フォルダ形式 — .mdファイル + images/ ディレクトリ）
- PDFエクスポートに画像埋め込み（knot-image:// URLを復号済みバイナリに変換）
- バックリンクパネル（ノートにリンクしているノート一覧 — Wikilink機能の仕上げ）
- 文字数/単語数カウント（エディタ下部に表示）

**サブ機能（時間があれば）:**
- ノート複製
- 自動バックアップ（定期的に全ノート暗号化エクスポート）
...
- エディタ内インライン画像ウィジェット（ユーザー要望があれば検討）
- スペルチェック設定
