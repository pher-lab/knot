...
**判断**: `react-markdown` + `remark-gfm`でReact要素として直接レンダリング。`dangerouslySetInnerHTML`不使用。

**ウィキリンクの実装方式**:
- 前処理で`[[Title]]` → `[Title](#wikilink:encodedTitle)`に変換
- `react-markdown`のカスタム`components.a`で`#wikilink:`リンクを検出
- `<a>`ではなく`<span role="link">`でレンダリング（Tauriのリンクハンドリング回避）
- `wikilink://`カスタムプロトコル方式は不採用: `react-markdown`のURLサニタイザーが`http/https/mailto/tel`以外のプロトコルを除去するため

**CodeMirror保持戦略**:
- プレビュー切替時にCodeMirrorのdivをunmountしない（`display:none`で隠す）
- 理由: EditorView初期化エフェクトは`currentNote?.id`に依存しており、unmount→remountすると再初期化されない。undo履歴も消える

**改行・空白の扱い**:
- `white-space: pre-wrap`で単一改行と連続スペースを保持
- `remark-breaks`は不採用: `pre-wrap`と併用すると`<br>`+テキストノード内`\n`で二重改行になる
- `overflow-wrap: break-word`でスペースなし長文字列のオーバーフローを防止

## Tauri WebViewでのalert()の問題

**判断**: `window.alert()`を使わず、すべてのユーザー通知にカスタムダイアログ（`ConfirmDialog`）を使用する。

**理由**:
- TauriのWebView2（Windows）で`window.alert()`が表示されないケースが確認された
- Welcomeノート復元時の「既に存在します」通知が全く表示されない現象
- `window.confirm()`も同様の問題があり、v0.3.0でカスタム`ConfirmDialog`に置き換え済み（L-10）

**方針**:
- ユーザーへの通知はすべて`ConfirmDialog`コンポーネントまたはUIバナーで行う
- `alert()`/`confirm()`/`prompt()`はTauri環境では使用しない
